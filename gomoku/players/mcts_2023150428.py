...
교수님께서 ipynb파일에 올려주신 Guideline에 포함된 GomokuBoard, GameResult, CellState class를 import하여 사용하는 방식으로 구현하였습니다.
번거롭지만 이 파일에 위 class를 import하여 사용해주시면 감사하겠습니다.
이러한 방식으로 제출한 이유는 Enum을 상속한 GameResult, CellState class가 다른 파일에도 정의가 되어 있으면 오류가 발생하여 부득이하게 이렇게 제출하게 되었습니다.

다시 한 번 번거롭게 하여 죄송합니다.
